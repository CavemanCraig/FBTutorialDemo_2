...
public class MyWebServiceTest {
	...
	public void setup() throws Exception {
		...
		friendIDList.add("\"newArray\"");
		TestUtils.createPlayerWithFriends(1000, "Ihave%20ZeroFriends",
				friendIDList);
		...
		TestUtils.createPlayerWithFriends(1001, "Ihave%20OneFriends",
				friendIDList);
		...
		TestUtils.createPlayerWithFriends(1004, "Ihave%20FourFriends",
				friendIDList);
		...
		TestUtils.createPlayerWithFriends(1005, "Ihave%20FiveFriends",
				friendIDList);
		...
		TestUtils.createPlayerWithFriends(1010, "Ihave%20TenFriends",
				friendIDList);
	}
	...
	public void testSetupData() {
		...
		User twoFriend = TestUtils.getUser(playerWith5Friends.getFriendList()
				.get(1));
	...
	public void testGameLinkForNotEnoughFriends() {
		...
		String expectedOnCLickMethod = "(function (){alert('You do not have enough friends to play the game.');return false;});";
		Assert.assertTrue(playerWith0Friends.getFriendList().size() == 0);
		Assert.assertTrue(playerWith0Friends.getGameLink().getOnClickMethod()
				.equals(expectedOnCLickMethod));
		Assert.assertTrue(playerWith0Friends.getGameLink().getHref()
				.equals("index.html"));
		...
		Assert.assertTrue(playerWith4Friends.getFriendList().size() == 4);
		Assert.assertTrue(playerWith4Friends.getGameLink().getOnClickMethod()
				.equals(expectedOnCLickMethod));
		Assert.assertTrue(playerWith4Friends.getGameLink().getHref()
				.equals("index.html"));
	}
	...
	public void testGameLinkForValidNumOfFriends() {
		...
		String expectedHrefBeginning = "playGame.html?playerID="
				...
				+ "&playerName=" + playerWith5Friends.getPlayerInfo().getName()
				+ "&playerPoints=" + playerWith5Friends.getPoints()
				+ "&friendIDList=";
		Assert.assertTrue(playerWith5Friends.getFriendList().size() == 5);
		Assert.assertTrue(playerWith5Friends.getGameLink().getOnClickMethod()
				.equals(""));
		Assert.assertTrue(playerWith5Friends.getGameLink().getHref()
				.startsWith(expectedHrefBeginning));
		...
		Assert.assertTrue(isValidGameLinkLists(expectedHrefBeginning,
				playerWith5Friends));
		...
		Assert.assertTrue(playerWith10Friends.getFriendList().size() == 10);
		Assert.assertTrue(playerWith10Friends.getGameLink().getOnClickMethod()
				.equals(""));
		Assert.assertTrue(playerWith10Friends.getGameLink().getHref()
				.startsWith(expectedHrefBeginning));
		Assert.assertTrue(isValidGameLinkLists(expectedHrefBeginning,
				playerWith10Friends));
	}
	...
	private boolean isValidGameLinkLists(String HrefBeginning, Player player) {
		...
		int friendIDEnd = player.getGameLink().getHref()
				.indexOf("&friendNameList=");
		String friendIDs = player.getGameLink().getHref()
				.substring(friendIDStart, friendIDEnd);
		int friendNameStart = player.getGameLink().getHref()
				.indexOf("friendNameList=") + 15; // we need to add
													// len("friendNameList=")
		String friendNames = player.getGameLink().getHref()
				.substring(friendNameStart);
		...
		for (String friendID : friendIDList) {
			...
			if (player.getFriendList().contains(Long.valueOf(friendID)) == false) {
				System.out.println("ID [" + friendID
						+ "] is not among the players friendlist"
	...
	public void testSubmitAllWrongAnswers() {
		...
		// Submit 3 wrong answers to our WebService as a POST request
		String targetURL = "http://localhost:8080/FBTutorialDemo/rest/webService/GameAnswers/" + "1005/67890/76543/89012";
		String JSONInput = "[\"" + "Four Friend" + "\",\"" + "Five Friend"
				+ "\",\"" + "One Friend" + "\"]";
